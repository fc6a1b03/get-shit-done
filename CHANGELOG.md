...
### Added
- **Kimi runtime support** — First-class Kimi CLI runtime support. Installs skills to `~/.kimi/skills/gsd-*/SKILL.md` and agents to `~/.kimi/agents/gsd-*/` as `agent.yaml` + `system.md` + `sub.yaml`. Maps Claude tool names to `kimi_cli.tools.*` namespace. `--kimi` flag with `KIMI_CONFIG_DIR` env var support.
