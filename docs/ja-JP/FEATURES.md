...
**要件:**
- REQ-RUNTIME-01: システムは Claude Code、OpenCode、Gemini CLI、Kilo、Codex、Copilot、Antigravity、Trae、Kimi、Cline、Augment Code、CodeBuddy、Qwen Code をサポートしなければならない
...
| 側面 | Claude Code | OpenCode | Gemini | Kilo | Codex | Copilot | Antigravity | Trae | Kimi | Cline | Augment | CodeBuddy | Qwen Code |
|------|------------|----------|--------|-------|-------|---------|-------------|------|------|-------|---------|-----------|-----------|
| コマンド | スラッシュコマンド | スラッシュコマンド | スラッシュコマンド | スラッシュコマンド | スキル（TOML） | スラッシュコマンド | スキル | スキル | スキル | ルール | スキル | スキル | スキル |
| エージェント形式 | Claude ネイティブ | `mode: subagent` | Claude ネイティブ | `mode: subagent` | スキル | ツールマッピング | スキル | スキル | YAML + MD | ルール | スキル | スキル | スキル |
| フックイベント | `PostToolUse` | N/A | `AfterTool` | N/A | N/A | N/A | N/A | N/A | N/A | N/A | N/A | N/A | N/A |
| 設定 | `settings.json` | `opencode.json(c)` | `settings.json` | `kilo.json(c)` | TOML | Instructions | Config | Config | `agent.yaml` | `.clinerules` | Config | Config | Config |
