...
**요구사항.**
- REQ-RUNTIME-01: Claude Code, OpenCode, Gemini CLI, Kilo, Codex, Copilot, Antigravity, Trae, Kimi, Cline, Augment Code, CodeBuddy, Qwen Code를 지원해야 합니다.
...
| 측면 | Claude Code | OpenCode | Gemini | Kilo | Codex | Copilot | Antigravity | Trae | Kimi | Cline | Augment | CodeBuddy | Qwen Code |
|--------|------------|----------|--------|-------|-------|---------|-------------|------|------|-------|---------|-----------|-----------|
| 명령어 | 슬래시 명령어 | 슬래시 명령어 | 슬래시 명령어 | 슬래시 명령어 | Skills(TOML) | 슬래시 명령어 | Skills | Skills | Skills | Rules | Skills | Skills | Skills |
| 에이전트 형식 | Claude native | `mode: subagent` | Claude native | `mode: subagent` | Skills | Tool mapping | Skills | Skills | YAML + MD | Rules | Skills | Skills | Skills |
| 훅 이벤트 | `PostToolUse` | N/A | `AfterTool` | N/A | N/A | N/A | N/A | N/A | N/A | N/A | N/A | N/A | N/A |
| 구성 | `settings.json` | `opencode.json(c)` | `settings.json` | `kilo.json(c)` | TOML | Instructions | Config | Config | `agent.yaml` | `.clinerules` | Config | Config | Config |
